...
对象成员的**构造函数**是在对象自己的构造函数之前被调用的。

成员**析构函数**是在类对象的析构函数之后调用的。
...
x被复制到y后，它们是等价且独立的。也就是说，在复制后x==y是true（等价性），对x的修改不会引起对y的修改（独立性）。
...
最佳实践是要使用`default`关键字显式声明默认的复制赋值运算符和复制构造函数。

有些类根本不能或不应该被复制，例如，管理着文件的类或代表并发编程中的互斥锁的类。可以使用`delete`关键字阻止编译器生成复制构造函数和复制赋值运算符。
...
❑ 独立性：在复制赋值或复制构造之后，原始对象和副本对象在修改过程中**不应该**改变对方的状态。

❑ 等价性：原始对象和副本对象应该是一样的。等价性的语义取决于上下文。一般来说，应用于原始对象的操作在应用于副本对象时应该产生相同的效果。

# 移动语义

不能使用**移出**状态的对象，除非释放或销毁它们。

## 值类别

每个表达式都有两个重要的特征：它的**类型**和**值类别**。值类别描述了什么样的操作对该表达式有效。

**左值**是有名字的值，而**右值**是非左值的值。不是以`=`分界的！！

## 左值引用和右值引用

左值引用，用一个`&`表示。我们也可以使用`&&`来接受一个参数右值引用。

## std::move函数

可以使用`<utility>`头文件中的`std::move`函数将左值引用转换成右值引用。

## 移动构造

移动构造函数看起来像复制构造函数，只是它们采用**右值引用**而非左值引用。

移动构造函数被设计成不会抛出异常，所以我们可以把它标记为`noexcept`。

通常，编译器不能使用抛出异常的移动构造函数，而会使用复制构造函数。因为编译器更喜欢慢但正确的代码，而不是快但不正确的代码。

## 移动赋值

我们也可以通过`operator=`创建类似于复制赋值运算符的移动赋值运算符。

移动赋值运算符接受右值引用，而不是const型左值引用，我们通常将它标记为`noexcept`。
...
在某些情况下，编译器可以为每个方法生成默认实现。不幸的是，生成方法的规则很复杂，而且在不同的编译器中可能是不一致的。我们可以通过将这些方法设置为`default/delete`或通过恰当的方式实现它们来消除这种复杂性。

![编译器生成方法](images/2024-06-01编译器生成方法.png)
